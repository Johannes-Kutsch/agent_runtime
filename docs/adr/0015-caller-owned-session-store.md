...
## Amendment: Absent local state raises ContinuationUnrecoverableError; "No conversation found" is the residual Claude trigger

`already in use` was not a stale server-side lock. It was `ar` **self-inflicting the collision on every retry**: `resolve_claude_resumed_session_facts` downgraded a resume to a fresh start when the probe dir looked empty (`RunKind.FRESH`) while keeping the continuation's original session id, so rendering re-emitted `claude --session-id <original-id>` every retry. The string was a symptom of an `ar` bug (fresh start through a used id), not a dead session.

**Structural fix.** Absent local session state must **raise `ContinuationUnrecoverableError`** — never downgrade to a fresh start, never reuse the continuation's id for a fresh start. Codex already did this. Claude and OpenCode are brought into line (OpenCode gains the pre-flight `_opencode_is_resumable` guard); all three now behave identically: absent local state ⇒ raise, consumer re-plans. A genuinely new session always gets a freshly-minted, never-before-used id, so `already in use` is structurally impossible from a correct runtime; if it surfaces it is a plain hard error, not `ContinuationUnrecoverableError`.

**Residual detection.** One case escapes the pre-flight check: local state and token are valid and the probe passes, yet the id is absent in the provider home Claude actually read (e.g. a consumer overrides `CLAUDE_CONFIG_DIR` so ar's probe dir and Claude's real home diverge). Claude reveals this by replying `No conversation found with session ID: <uuid>`. Built-in Provider Stream Interpretation's Claude-specific reducer recognizes that signal and raises `ContinuationUnrecoverableError` instead of a generic `HardAgentError`. The `already in use` detection path is dropped — that string is now prevented, not detected.

Resumed-session failures remain non-invalidating by default; only the two named triggers raise `ContinuationUnrecoverableError`. `ContinuationUnrecoverableError` carries optional `classification` (which signal fired) and `raw_message` (exact provider text). The stream trigger is Claude-only; OpenCode and Codex rely on the pre-flight guard.

## Amendment: The provider home is the Session Store root — drop the `implementer/<provider>` segment

The prior amendment improved the error mode, but resume still failed for consumers that own the provider's config directory. ar nested provider state under `<session_store>/implementer/<provider>/`. When a consumer sets `CLAUDE_CONFIG_DIR` / `CODEX_HOME` / `OPENCODE_HOME` directly to `session_store` (e.g. a downstream argv-transform consumer that bind-mounts `session_store` into a container), the provider wrote its transcript to `session_store` while ar probed `session_store/implementer/<provider>` — an empty dir — and raised `ContinuationUnrecoverableError` on every resume.

`"implementer"` was never a runtime concept. It is a pycastle role that leaked across the boundary into ar's on-disk contract, violating the rule that the runtime uses runtime-neutral names and owns no role concept. Option A (thread the real role/namespace through the resolvers) would import a foreign role concept onto ar's public request surface; Option B (drop the segment) removes a concept ar should never have had.

**Decision: Option B.** The provider's isolated home **is** the `session_store` root. ar `mkdir`s, seeds credentials, probes, and decides resumability **directly on `session_store`** — no owner, provider, or namespace subdirectory. ar therefore probes exactly where the provider reads/writes, so resume with an existing transcript resolves to `RunKind.RESUME` instead of downgrading to `FRESH`. One `session_store` holds one provider's home; scoping per session/role is the caller's responsibility, not ar's.

The `Continuation`'s `provider_state_dir_relpath` field survives, now optionally empty: empty means the Store root is the home (all new continuations); non-empty resolves legacy `implementer/<provider>` tokens via the existing join-stored-relpath logic. Clean break — no migration code; legacy tokens resume correctly for free.

The pre-flight raise-on-absent-state guard and the Claude `No conversation found` stream trigger are unchanged: once ar probes where the provider actually writes, the pre-flight check passes on a genuine resume, and the two error triggers fire only when session state is truly gone.
