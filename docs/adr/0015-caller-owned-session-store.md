...
## Amendment: A second trigger — Claude reporting a dead session mid-invocation

The prior amendment covers only the case where Session-backed Provider State Resolution can tell *before* invoking the provider that the session is gone (rollout paths present, no recoverable session id). A different failure shape exists: the provider CLI itself is invoked against a session id that turns out to be dead, and says so directly. For Claude this happens two ways, both observed from a real incident (pycastle#1946 — a Claude Code subprocess was killed mid-turn by a mid-turn credit cutoff, leaving `ar`'s continuation pointed at a session the provider could no longer honor):

- `Session ID <uuid> is already in use.` — a stale server-side lock from the killed subprocess, held for as long as ~19 hours in the observed incident.
- `No conversation found with session ID: <uuid>` — the same session once the stale lock itself expired.

Neither is reachable from Session-backed Provider State Resolution's pre-flight filesystem check: the local Session Store and continuation token are both perfectly valid; only the provider's own server-side/session-lock state is gone, and the provider only reveals that by being invoked. So the second trigger point lives one layer up, in Built-in Provider Stream Interpretation's Claude-specific reducer: it recognizes this closed, Claude-only set of provider-reported "the session is gone" signals (loose substring match on the CLI's known phrasing, both as a JSON `result` event field and as a bare non-JSON stderr line — the two signals arrive differently) and raises `ContinuationUnrecoverableError` from there instead of letting the failure fall through to a generic, unclassified `HardAgentError` (or, in one case, an existing pre-existing misclassification as `TransientAgentError`).

We deliberately did not generalize this to "any Resume Session Run failure is unrecoverable." `Resumed-session failures do not invalidate continuations or trigger automatic Consumer Fallback inside runtime` remains the default — a transient hiccup (rate limit, network blip, timeout) must not discard a perfectly resumable continuation. Only this closed, explicitly-recognized signal set raises `ContinuationUnrecoverableError` from stream interpretation; everything else keeps its existing classification.

`ContinuationUnrecoverableError` now also carries optional `classification` (which of the two known signals fired) and `raw_message` (the exact provider text), mirroring the shape already used by `HardAgentError.classification` and `ModelNotAvailableError.raw_message`, so a consumer catching it has structured access to both without parsing `str(exc)`.

This detection is Claude-only for now. Codex already has its own pre-flight mechanism (the first amendment, above); OpenCode has no observed analogous signal. Extending to either is deferred until a real gap is observed there, not built speculatively.
