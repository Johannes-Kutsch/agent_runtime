...
- [0014 - Windows provider host environment allowlist](0014-windows-provider-host-env-allowlist.md): Restore a five-key Windows host-env allowlist (not a full merge), layered once at the invocation boundary for every built-in provider; Windows-only, no-op on POSIX.
- [0015 - Caller-owned Session Store](0015-caller-owned-session-store.md): Resume state lives in a caller-owned, isolated Session Store (required for session-backed runs); the runtime never serializes/embeds/recreates provider state; uniform across providers.
- [0016 - ToolPolicy: remove INSPECT_ONLY](0016-toolpolicy-remove-inspect-only.md): `INSPECT_ONLY` retired; ToolPolicy narrows to three values: `NONE`, `NO_FILE_MUTATION`, `UNRESTRICTED`.
- [0017 - Already-Sandboxed Execution](0017-already-sandboxed-execution.md): `already_sandboxed` flag at `RuntimeClient` constructor skips Codex OS sandbox creation when host (e.g. Docker container) already provides isolation; Claude and OpenCode unaffected.
