...
- [0018 - Execution Argv Transform replaces Already-Sandboxed Execution](0018-execution-argv-transform.md): Per-invocation argv/cwd/env transform on all three run request types routes execution into containers without ar gaining a Docker dependency; supersedes 0017.
- [0019 - AgentFailedError belongs to consumers, not ar](0019-retire-agent-failed-error-from-ar.md): Retire unused `AgentFailedError` from ar's public surface; consumers own their own error types.
- [0020 - Wiring CancellationToken into invocation execution](0020-cancellation-token-wiring.md): Poll sub-second, hard-kill only, progress-gated continuation preservation mirroring Idle Timeout; `.cancel()` designed for cross-thread use, token switches to `threading.Event`, execution stays synchronous.
