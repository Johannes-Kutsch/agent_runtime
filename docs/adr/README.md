...
- [0017 - Execution Argv Transform](0017-execution-argv-transform.md): Per-invocation argv/cwd/env transform on all three run request types routes execution into containers without ar gaining a Docker dependency; replaces the retired `already_sandboxed` constructor flag.
- [0018 - AgentFailedError belongs to consumers, not ar](0018-retire-agent-failed-error-from-ar.md): Retire unused `AgentFailedError` from ar's public surface; consumers own their own error types.
- [0019 - Wiring CancellationToken into invocation execution](0019-cancellation-token-wiring.md): Poll sub-second, hard-kill only, progress-gated continuation preservation mirroring Idle Timeout; `.cancel()` designed for cross-thread use, token switches to `threading.Event`, execution stays synchronous.
- [0020 - TransientAgentError retired](0020-retire-transient-agent-error.md): Retire `TransientAgentError`; `status_code` leaves ar's exception surface. Routing claim refined by 0021.
- [0021 - TransientError is retryable by definition](0021-transient-error-is-retryable-by-definition.md): The classification gate was a category error; `TransientError` always surfaces as `ProviderUnavailable`, `classification` deleted from the contract, per-service divergence on status-less failures deliberately left alone.
