...
Status: Refined by [0011 - Single-candidate provider selection](0011-single-candidate-provider-selection.md) for resume provider identity and request-time credentials.
...
- Let resumed-session execution derive service, model, effort, and tool policy from the continuation while receiving credentials separately.
