...
- Session-backed execution is in scope for all built-in providers and must preserve provider-native transcript continuity. New-session and resumed-session calls require a caller-supplied `RuntimeStateDir`; ephemeral calls do not.
- Continuations carry provider state identifiers relative to `RuntimeStateDir`, not absolute provider state paths.
- Durable invocation logging is opt-in through a separate `RuntimeLogsDir`; omitted logs do not create hidden log files by default.
