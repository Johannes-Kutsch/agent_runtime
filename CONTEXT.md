...
| `Continuation` | Opaque portable resume token callers persist and pass back to resume a continuity chain. Carries resume identity and a pointer (relative path) into the caller's `Session Store`; not the provider session bytes themselves. |
| `Session Store` | Caller-owned, isolated directory holding a session-backed provider's native on-disk session state. Supplied symmetrically to `Start Session Run` and `Resume Session Run` so a `Continuation` can resume. The runtime points the provider's home environment at it and reads/writes within it, but never owns, creates durably, cleans, serializes, or recreates it. Distinct from the host user's native provider home. |
...
- The runtime never serializes, embeds, or recreates provider session state. Session-backed runs (Start Session Run, Resume Session Run) require a caller-owned Session Store, supplied symmetrically across new and resume; the runtime points the provider home at the Store and lets the provider resolve its own session by id. Omitting the Store on a session-backed run is a configuration error. Every Session-backed Provider's Continuation carries a relative-path pointer into the Store.
...
- The runtime raises `ContinuationUnrecoverableError` — not `RuntimeConfigurationError` — whenever provider-side session state is confirmed gone despite a valid `Continuation`, from either of two trigger points: Session-backed Provider State Resolution's pre-flight filesystem check (valid token present, provider-side session state absent), or Built-in Provider Stream Interpretation recognizing one of a closed set of provider-reported "this session is gone" signals during a Claude invocation (currently: a session-conflict or session-not-found CLI signal). Consumers own the decision to drop the continuation and re-plan in both cases.
