...
| `ServiceName` | A path-safe runtime service identity used for selection, invocation records, and diagnostics. |
...
| `ProviderInvocationRequest` | Private value describing all facts required by the Built-In Provider Invocation Seam: command, invocation directory, environment, prompt content or prompt-path policy, `RunKind`, optional `ProviderSessionId`, and invocation metadata used for diagnostics. |
...
| `Tool-less Run` | A runtime invocation whose `ToolPolicy` explicitly forbids provider tools rather than leaving tools to provider defaults. |
...
| `RuntimeStateDir` | Transitional caller-supplied directory root previously used for provider-native session state; active session-backed requests do not require it. |
| `RuntimeLogsDir` | Transitional caller-supplied directory root previously used for runtime invocation logs; callers now own persistence for any durable traces. |
...
- Ordinary runtime execution uses a Built-in Execution Substrate; application-owned Docker orchestration, dependency installation, execution-directory management, and preflight setup remain outside the runtime boundary.
- Built-in provider subprocess dispatch, prompt-file lifecycle, and invocation-record interaction belong behind the Built-In Provider Invocation Seam, while provider-specific command rendering and lifecycle policy remain runtime-owned internals.
...
- Resumed-session execution keeps service and `ToolPolicy` fixed while defaulting model and effort from the continuation and allowing explicit model or effort overrides.
...
- Tool-less execution means provider tools are explicitly forbidden at the runtime boundary.
...
- Workspace access belongs to tool-policy configuration rather than session lifecycle.
...
- Canonical runtime requests should represent session intent and tool policy as explicit mode values rather than nullable optional fields.
